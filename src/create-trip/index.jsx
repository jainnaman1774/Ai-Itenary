...
function CreateTrip() {
  ...
  useEffect(() => {
    if (!import.meta.env.VITE_GOOGLE_PLACE_API_KEY) {
      console.warn('Missing VITE_GOOGLE_PLACE_API_KEY in .env')
    }
  }, [])
      ...
      <div className='mt-20 flex flex-col gap-10'>
        <div>
          ...
          <GooglePlacesAutocomplete
            ...
            onLoadFailed={(error) => {
              console.error('Google Places load failed:', error)
              setPlacesApiError(error?.message || 'Failed to load Google Places API')
            }}
            selectProps={{
              value: place,
          ...
          />
          {placesApiError && (
            <p className='text-sm text-red-500 mt-2'>Google Places API error: {placesApiError}</p>
          )}
        </div>
